    ...
    <div style={{fontFamily:FF,background:K.paper,minHeight:"100vh",color:K.ink,maxWidth:"500px",margin:"0 auto"}}>
      ...
      <div style={{background:"#ede9e0",borderBottom:`1px solid ${K.border}`,padding:"12px 14px"}}>
        ...
        <div style={{marginBottom:"9px"}}>
          <div style={{fontSize:"10px",color:K.muted,letterSpacing:".1em",textTransform:"uppercase",fontWeight:600,marginBottom:"5px"}}>상대 직급</div>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"5px"}}>
            ...
            <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
              {(fRank||fSize||fGender||fMeal||fAlcohol||showFavOnly)&&<button onClick={()=>{setFRank("");setFSize("");setFGender("");setFMeal("");setFAlcohol("");setShowFavOnly(false);}} style={{fontSize:"11px",color:K.accent,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:700}}>필터 초기화</button>}
          ...
          <div style={row}>
            ...
            {regionList.map(r=>{const cnt=restaurants.filter(x=>x.region===r).length;return(<button key={r} style={regionChip(fRegion===r)} onClick={()=>setFRegion(r)}>{r}{cnt>0?` (${cnt})`:""}</button>);})}
            <button style={{...regionChip(false),borderStyle:"dashed"}} onClick={()=>{const c=prompt("새 지역 이름:");if(c?.trim()) saveExtraRegion(c.trim());}}>+ 추가</button>
          </div>
        </div>
      </div>
