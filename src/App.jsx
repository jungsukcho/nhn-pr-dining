...
const Stars = ({ value, onChange }) => (
  <span style={{display:"flex",gap:"2px"}}>
    {[1,2,3,4,5].map(i=>(
      <span key={i} onClick={()=>onChange&&onChange(i)} style={{fontSize:"15px",cursor:onChange?"pointer":"default",color:i<=value?"#c8900a":"#d8d0c4",transition:"color .12s"}}>★</span>
    ))}
  </span>
);

const Tag = ({label,bg="#ede9e0",color="#6a5e4e",border,small=false})=>(
  <span style={{background:bg,color,fontSize:small?"10px":"11px",fontWeight:600,padding:small?"2px 6px":"3px 9px",borderRadius:"20px",whiteSpace:"nowrap",lineHeight:1.4,border:`1px solid ${border||bg}`}}>{label}</span>
);
...
export default function App() {
  ...
  if(loading) return (
    <div style={{fontFamily:FF,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:K.paper,color:K.muted,gap:"16px"}}>
      ...
      <div style={{fontFamily:FF,background:K.paper,minHeight:"100vh",color:K.ink,maxWidth:"500px",margin:"0 auto"}}>
       ...
       {showDetailFilter&&<div style={{}}>
        ...
        <div style={sec}>
          ...
          <div style={{...row,marginBottom:"14px"}}>{MOOD_OPTS.map(a=>(<button key={a} style={mChip(form.ambiance?.includes(a))} onClick={()=>toggleArr("ambiance",a)}>{a}</button>))}</div>
          <label style={fLabel}>시설</label>
          <div style={{...row,marginBottom:"14px"}}>{FACILITY_OPTS.map(a=>(<button key={a} style={mChip(form.ambiance?.includes(a))} onClick={()=>toggleArr("ambiance",a)}>{a}</button>))}</div>
    ...
    <div style={{fontFamily:FF,background:K.paper,minHeight:"100vh",color:K.ink,maxWidth:"500px",margin:"0 auto"}}>
      <div style={{background:K.night,borderBottom:`3px solid ${K.accent}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"15px 16px 12px"}}>
          <div style={{cursor:"pointer"}} onClick={()=>{setView("list");setSelected(null);setForm(null);setFRegion("전체");setFRank("");setFSize("");setFGender("");setFMeal("");setFAlcohol("");setFSearch("");setShowFavOnly(false);setActiveGroup("전체");}}>
      ...
      <div style={{background:"#ede9e0",borderBottom:`1px solid ${K.border}`,padding:"12px 14px"}}>
        ...
        <div style={{marginBottom:"10px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
            <div style={{display:"flex",gap:"6px"}}>
              <button onClick={()=>{setActiveGroup("전체");setFRegion("전체");}} style={{padding:"5px 12px",borderRadius:"20px",fontSize:"12px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:activeGroup==="전체"?`2px solid ${K.accent}`:`1px solid #c0b8a8`,background:activeGroup==="전체"?K.accent:"#fff",color:activeGroup==="전체"?"#fff":K.muted}}>전체</button>
              {Object.keys(REGION_GROUPS).map(g=>(<button key={g} onClick={()=>{setActiveGroup(activeGroup===g?"전체":g);setFRegion("전체");}} style={{padding:"5px 12px",borderRadius:"20px",fontSize:"12px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .12s",border:activeGroup===g?`2px solid ${K.night}`:`1px solid #c0b8a8`,background:activeGroup===g?K.night:"#f0ece4",color:activeGroup===g?K.gold:K.muted}}>{g}</button>))}
            </div>
          ...
          {Object.entries(REGION_GROUPS).filter(([g])=>activeGroup==="전체"||activeGroup===g).map(([groupName,groupRegions])=>{const groupInList=regionList.filter(r=>groupRegions.includes(r));const isExpanded=expandedGroups[groupName];const visible=isExpanded?groupInList:groupInList.slice(0,3);const hasMore=groupInList.length>3;return(<div key={groupName} style={{marginBottom:"8px"}}><div style={{display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap"}}>{activeGroup==="전체"&&<span style={{fontSize:"10px",fontWeight:800,color:K.gold,letterSpacing:".08em",minWidth:"52px"}}>{groupName}</span>}{visible.map(r=>{const cnt=restaurants.filter(x=>x.region===r).length;return(<button key={r} style={regionChip(fRegion===r)} onClick={()=>setFRegion(fRegion===r?"전체":r)}>{r}{cnt>0?` (${cnt})`:""}</button>);})}{hasMore&&(<button onClick={()=>setExpandedGroups(prev=>({...prev,[groupName]:!prev[groupName]}))} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:`1px dashed #c0b8a8`,background:"#fff",color:K.muted}}>{isExpanded?"접기":`+${groupInList.length-3}더보기`}</button>)}</div></div>);})}
          {activeGroup==="전체"&&(()=>{const allGrouped=Object.values(REGION_GROUPS).flat();const others=regionList.filter(r=>!allGrouped.includes(r));if(others.length===0) return null;const isExpanded=expandedGroups["기타"];const visible=isExpanded?others:others.slice(0,3);return(<div style={{marginBottom:"8px"}}><div style={{display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap"}}><span style={{fontSize:"10px",fontWeight:800,color:K.gold,letterSpacing:".08em",minWidth:"52px"}}>기타</span>{visible.map(r=>{const cnt=restaurants.filter(x=>x.region===r).length;return(<button key={r} style={regionChip(fRegion===r)} onClick={()=>setFRegion(fRegion===r?"전체":r)}>{r}{cnt>0?` (${cnt})`:""}</button>);})}{others.length>3&&(<button onClick={()=>setExpandedGroups(prev=>({...prev,"기타":!prev["기타"]}))} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:`1px dashed #c0b8a8`,background:"#fff",color:K.muted}}>{isExpanded?"접기":`+${others.length-3}더보기`}</button>)}</div></div>);})()}
        </div>
        <div style={{marginBottom:"10px"}}>
          <div style={{fontSize:"10px",color:K.muted,letterSpacing:".1em",textTransform:"uppercase",fontWeight:600,marginBottom:"6px"}}>식사 시간</div>
      ...
      <button onClick={()=>openAdd()} style={{position:"fixed",bottom:"24px",right:"20px",background:K.accent,color:"#fff",border:"none",borderRadius:"50px",padding:"13px 22px",fontWeight:800,cursor:"pointer",fontSize:"14px",fontFamily:"inherit",boxShadow:"0 4px 16px rgba(184,40,0,.35)",zIndex:100,letterSpacing:".04em"}}>+ 음식점 추가</button>
      <Toast/>
    </div>
  );
}
