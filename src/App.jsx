        ...
        <div style={{marginBottom:"10px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
            <div style={{display:"flex",gap:"6px"}}>
              ...
              {Object.keys(REGION_GROUPS).map(g=>{const isActive=regionList.filter(r=>REGION_GROUPS[g].includes(r)).some(r=>r===fRegion);return(<button key={g} onClick={()=>setFRegion(fRegion===g?"전체":g)} style={{padding:"5px 12px",borderRadius:"20px",fontSize:"12px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .12s",border:fRegion===g?`2px solid ${K.night}`:`1px solid #c0b8a8`,background:fRegion===g?K.night:"#f0ece4",color:fRegion===g?K.gold:K.muted}}>{g}</button>);})}
            </div>
