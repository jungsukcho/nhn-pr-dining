...
export default function App() {
  ...
  const hasDetailFilter = fRank||fSize||fGender||fMeal||fAlcohol;
    ...
    <div style={{fontFamily:FF,background:K.paper,minHeight:"100vh",color:K.ink,maxWidth:"500px",margin:"0 auto"}}>
      <div style={{background:K.night,borderBottom:`3px solid ${K.accent}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"15px 16px 12px"}}>
          <div style={{cursor:"pointer"}} onClick={()=>{setView("list");setSelected(null);setForm(null);setFRegion("전체");setFRank("");setFSize("");setFGender("");setFMeal("");setFAlcohol("");setFSearch("");setShowFavOnly(false);setActiveGroup("전체");setShowDetail(false);}}>
        ...
        <div style={{marginBottom:"8px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
            ...
            <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
              {(hasDetailFilter||showFavOnly)&&(<button onClick={()=>{setFRank("");setFSize("");setFGender("");setFMeal("");setFAlcohol("");setShowFavOnly(false);}} style={{fontSize:"11px",color:K.accent,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:700}}>초기화</button>)}
          ...
          {/* 상세조건 버튼 */}
          <button onClick={()=>setShowDetail(!showDetail)} style={{marginTop:"8px",width:"100%",padding:"8px",borderRadius:"8px",border:`1px solid ${hasDetailFilter?K.accent:"#c0b8a8"}`,background:showDetail?K.night:"#fff",color:showDetail?"#eeeae2":hasDetailFilter?K.accent:K.muted,fontSize:"12px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",letterSpacing:".04em"}}>
            {showDetail?"▲ 상세조건 닫기":`▼ 상세조건${hasDetailFilter?" (적용중)":""}`}
          </button>
        </div>

        {/* 상세 필터 — 펼쳐질 때만 표시 */}
        {showDetail&&(
          <div style={{borderTop:`1px solid ${K.border}`,paddingTop:"10px"}}>
            <div style={{marginBottom:"10px"}}>
              ...
              <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
                ...
                {(fMeal||fAlcohol)&&<button onClick={()=>{setFMeal("");setFAlcohol("");}} style={{...chip(false),border:`1px solid #c0b8a8`}}>✕ 해제</button>}
              </div>
            </div>
